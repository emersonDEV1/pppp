var targetCal;
function calendario(target, functionToCall, month = null, year = null){
    if(targetCal){
    ...
    calendarDiv += "<div class='close'><button type='button' title='Fechar' onclick='closeCalendario(\""+target+"\")'>X</button></div>";
    calendarDiv += "<div class='navigation'>";
    calendarDiv += "<button type='button' title='Mês anterior' class='navigator' onclick='calendario(\""+target+"\", \""+functionToCall+"\", "+mesPrev+", "+anoPrev+")'>&#10094;&#10094;</button>";
    calendarDiv += "<div class='tituloCalendario' onclick='calendario(\""+target+"\", \""+functionToCall+"\", "+mesAtual+", "+anoAtual+")'>"+mesExtenso+"</div>";
	calendarDiv += "<button type='button' title='Mês próximo' class='navigator' onclick='calendario(\""+target+"\", \""+functionToCall+"\", "+mesNext+", "+anoNext+")'>&#10095;&#10095;</button>";
    calendarDiv += "</div>";
    ...
    for(d = 1; d <= diasDoMes; d++){
        ...
        calendarDiv += "<button type='button' onclick='"+functionToCall+"(\""+dataGo+"\");closeCalendario(\""+target+"\")' class='diaCal"+current+"'"+disabled+">"+d+"</button>";
...
function closeCalendario(target){
